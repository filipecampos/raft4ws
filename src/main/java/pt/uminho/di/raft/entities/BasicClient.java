...
public class BasicClient extends DefaultClient {
	...
	private Integer num;
	protected Random r = new Random(System.nanoTime());
	...
	protected void insertNewDevice(EndpointReference epr, DeviceReference deviceReference) {
		...
		try {
			...
			{
				deviceReference = getDeviceReference(epr, CommunicationManagerRegistry.getPreferredCommunicationManagerID());
			}
	...
	protected void insertService(EndpointReference epr, Service svc) {
	...
	public LogEntry invokeInsertCommandOp(String command, String parameters) {
		...
		if (currentLeader != null) {
			...
			logger.warn("No leader to invoke InsertCommand operation!");
		}
	...
	public LogEntry invokeReadOp(String uid)
	{
		...
		if (currentLeader != null) {
			...
			logger.warn("No leader to invoke Read operation!");
		}
	...
	protected void setLeaderService(EndpointReference epr) {
		if (devicesInsertCommandOp.containsKey(epr)) {
			currentLeader = epr;
			logger.info("New leader with epr: " + epr + " is now set.");
		} else {
			logger.error("Couldn't retrieve new leader with epr: " + epr);
			ServiceReference svcRef = getServiceReference(epr, CommunicationManagerRegistry.getPreferredCommunicationManagerID());
			Service svc = null;
			try {
	...
	protected void execute() {
		...
		try {
			for (int i = 0; i < num; i++) {
				...
				while(!success)
				{
					LogEntry entry = invokeInsertCommandOp(command, parameters);
	...
	protected void connectToAnotherServer() {
		...
		devicesInsertCommandOp.remove(currentLeader);
		devicesReadOp.remove(currentLeader);
	
		Iterator<EndpointReference> iter = devicesInsertCommandOp.keySet().iterator();
		// select another leader
		if (iter.hasNext()) {
			setLeaderService(iter.next());
		} else {
			logger.error("No other known servers apart the failed leader " + currentLeader);
			currentLeader = null;
		}
	}
